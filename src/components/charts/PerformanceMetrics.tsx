  ...
  const metrics = [
    {
      ...
      change: parseFloat(calculateChange(latestData.성약률, previousData.성약률)),
      isReverse: false
    },
    {
      title: '평균 거래기간',
      value: `${latestData.평균거래기간}일`,
      change: parseFloat(calculateChange(latestData.평균거래기간, previousData.평균거래기간)),
      isReverse: true // 거래기간은 짧을수록 좋음
    },
    {
      title: '고객만족도',
      value: `${latestData.고객만족도}점`,
      change: parseFloat(calculateChange(latestData.고객만족도, previousData.고객만족도)),
      isReverse: false
    }
  ]
        ...
        <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.dataKey}: {entry.value}{entry.dataKey === '평균거래기간' ? '일' : entry.dataKey === '성약률' ? '%' : '점'}
            </p>
          ))}
        </div>
      )
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span className={`text-sm font-medium ${getChangeColor(metric.change, metric.isReverse)}`}>
