...
import { X, User, Mail, Phone, Building, Crown, Shield, Eye } from 'lucide-react'
...
interface MemberEditModalProps {
  ...
  onRoleChange: (memberId: string, newRole: TeamMember['role']) => Promise<void>
  onStatusChange: (memberId: string, newStatus: TeamMember['status']) => Promise<void>
...
export const MemberEditModal: React.FC<MemberEditModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'role' | 'status'>('info')
  ...
  const handleRoleChange = async (newRole: TeamMember['role']) => {
    if (!confirm(`${member.name}님의 역할을 ${ROLE_LABELS[newRole]}로 변경하시겠습니까?`)) return
  ...
  const canChangeRole = (targetRole: string) => {
    ...
    if (currentUserRole === 'admin' && ['member', 'viewer'].includes(targetRole)) return true
    return false
  }
          ...
          {activeTab === 'info' && (
            <div className="space-y-4">
              ...
              <div className="text-sm text-gray-500">
                <p>가입일: {new Date(member.joined_at).toLocaleDateString('ko-KR')}</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'role' && canEdit && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                현재 역할: <span className="font-medium">{ROLE_LABELS[member.role]}</span>
              </p>
              ...
              <div className="grid grid-cols-1 gap-3">
                {(['viewer', 'member', 'admin', 'owner'] as const).map((role) => (
                  <button
                    ...
                    disabled={!canChangeRole(role) || member.role === role || loading}
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-500">
                          {role === 'owner' && '모든 권한을 가지며 팀을 관리할 수 있습니다.'}
                          {role === 'admin' && '팀원을 관리하고 매물을 관리할 수 있습니다.'}
                          {role === 'member' && '매물을 등록하고 수정할 수 있습니다.'}
                          {role === 'viewer' && '매물을 조회만 할 수 있습니다.'}
                        </div>
                      </div>
                    </div>
                  </button>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">팀원이 정상적으로 활동할 수 있습니다.</div>
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">팀원이 일시적으로 비활성 상태입니다.</div>
                    </div>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500">팀원의 접근이 정지됩니다.</div>
                    </div>
                  </div>
                </button>
              </div>
